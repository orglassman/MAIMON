...
public class DependencySet implements IDependencySet {
    ...
    //OR: add method to compare two dependency sets
    public boolean isEqual(DependencySet dependency) {
        if (this.size != dependency.size())
            return false;

        if (!this.set.equals(dependency.set))
            return false;

        if (!this.inverseSet.equals(dependency.inverseSet))
            return false;
